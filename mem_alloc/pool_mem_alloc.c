...
void pool_init(Pool *p, void *backing_buffer, size_t backing_buffer_length, size_t chunk_size, size_t chunk_alignment) {
    // Align backing buffer to specified chunk chunk_alignment
    ...
    // Align chunk size up to required chunk_alignment
    ...
    assert(backing_buffer_length >= chunk_size && "Backing buffer length is smaller than chunk size");
