...
uint32_t *internal_node_child(void *node, uint32_t child_num) {
  ...
  if (child_num > num_keys) {
    printf("tried to acess child_num %d > num_keys %d\n", child_num, num_keys);
    exit(EXIT_FAILURE);
    ...
    if (*child == INVALID_PAGE_NUM) {
      printf("Tried to access chjild %d of node, but was invalid page\n",
             child_num);
      exit(EXIT_FAILURE);
    }
    return child;
...
void initialize_internal_node(void *node) {
  ...
  *internal_node_num_keys(node) = 0;
  ...
  /*
  Necesssary because the root page number is 0; by not initializing an internal
...
typedef enum {
  ...
  META_COMMAND_SUCCESS_UNRECOGNIZED_COMMAND
} MetaCommandResult;
...
Cursor *leaf_node_find(Table *table, uint32_t page_num, uint32_t key) {
  ...
  /*
  Either
  1. the position of the ky
  2. Position of another key
  3. The position one past the key
  */
...
MetaCommandResult do_meta_command(InputBuffer *input_buffer, Table *table) {
  if (strcmp(input_buffer->buffer, ".exit") == 0) {
    ...
    return META_COMMAND_SUCCESS_UNRECOGNIZED_COMMAND;
  }
}
...
PrepareResult prepare_insert(InputBuffer *input_buffer, Statement *statement) {
  ...
  // Q: Why strcpy vs direct assignment?
...
void create_new_root(Table *table, uint32_t right_child_page_num) {
  ...
  Old root compied to new page, becomes left child
  ...
  while (true) {
    ...
    if (input_buffer->buffer[0] == '.') {
      ...
      case (META_COMMAND_SUCCESS):
        continue;
      case (META_COMMAND_SUCCESS_UNRECOGNIZED_COMMAND):
